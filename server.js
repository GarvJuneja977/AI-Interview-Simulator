...
//       starterCode: {
//         javascript: `function mergeKLists(lists) {\n    // Your solution here\n    // Note: lists is an array of arrays for simplicity\n    \n}`,
//         python: `def merge_k_lists(lists: list[list[int]]) -> list[int]:\n    # Your solution here\n    # Note: lists is a list of lists for simplicity\n    pass`,
...
//   let questions = [];
//   if (difficulty && questionBank[difficulty.toLowerCase()]) {
//     questions = questionBank[difficulty.toLowerCase()];
//   } else {
//     questions = [...questionBank.easy, ...questionBank.medium, ...questionBank.hard];
//   }
//   res.json({ questions });
// });
...
//     return res.status(500).json({ error: 'API key not configured. Please add your ANTHROPIC_API_KEY to .env file.' });
//   }

//   try {
//     const prompt = `You are a technical interview evaluator. Evaluate the following solution for a coding problem.
...
// Time taken: ${Math.floor(timeTaken / 60)}m ${timeTaken % 60}s out of ${Math.floor(question.timeLimit / 60)}m allowed

// Please evaluate this solution and respond with ONLY a JSON object (no markdown, no extra text) in this exact format:
// {
//   "verdict": "Accepted" | "Wrong Answer" | "Time Limit Exceeded" | "Incomplete",
//   "score": <number 0-100>,
...
//   "sampleOptimalCode": "<clean optimal solution in ${language}>"
// }`;
...
//     res.status(500).json({ error: 'Failed to evaluate solution. ' + err.message });
//   }
// });
...
//       ? 'Give a very subtle hint about the approach without revealing the solution. Just point them in the right direction.'
//       : hintLevel === 2
...
//   const { question, code, userMessage, chatHistory = [] } = req.body;
//   if (!process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_API_KEY === 'your_anthropic_api_key_here') {
//     return res.status(500).json({ error: 'API key not configured.' });
//   }
//   try {
//     const systemPrompt = `You are Alex, a friendly but professional technical interviewer at a top software company. The candidate is solving: "${question?.title}". Ask follow-up questions about their approach, probe time/space complexity understanding, and give encouragement. Keep responses SHORT (2-3 sentences max). Never give away the solution. Current code: \`\`\`${code || '(none yet)'}\`\`\``;
...
//   console.log(`📝 Make sure you've added your ANTHROPIC_API_KEY to .env\n`);
// });
